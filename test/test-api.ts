 ...
 *
 * Usage: npx tsx --test test/test-api.ts
 */
...
async function waitForServer(maxWait = 45000): Promise<boolean> {
...
describe('API Endpoints', () => {
  ...
  it('PUT /api/todos/:id should update a todo', async () => {
    ...
    await fetch(`${BASE_URL}/api/todos/${created.id}`, {
      method: 'DELETE',
    });
  });
  ...
  it('GET /api/agent/:id/history should return messages', async () => {
    ...
    if (sys.agents.length > 0) {
      ...
      const resp = await fetch(
        `${BASE_URL}/api/agent/${agentId}/history`
      );
      assert.equal(resp.status, 200);
  ...
  it('GET /api/agent/nonexistent/history should return 404', async () => {
    const resp = await fetch(
      `${BASE_URL}/api/agent/nonexistent-agent-xyz/history`
    );
