 ...
 *
 * Usage: npx tsx --test test/test-rag.ts
 */
...
describe('RAG System', () => {
  ...
  it('should connect to ChromaDB', () => {
    assert.ok(
      memorySystem.chromaClient,
      'ChromaDB client should be initialized'
    );
  ...
  it('should add episodic memory', async () => {
    const id = await addEpisodicMemory(
      'test-agent',
      '你好',
      '你好！有什么可以帮你的？'
    );
  ...
  it('should search memory by relevance', async () => {
    ...
    assert.ok(
      topResult.content.includes('咪豆') ||
        topResult.content.includes('猫'),
  ...
  it('should return attention weights that sum to reasonable values', async () => {
    ...
    for (const r of results) {
      assert.ok(
        typeof r.attentionWeight === 'number',
        'Should have attention weight'
      );
      assert.ok(
        typeof r.metrics.timeDecay === 'number',
        'Should have time decay'
      );
  ...
  it('should cleanup old low-importance memories', async () => {
    const count = await cleanupMemories(0, 5);
    assert.ok(
      typeof count === 'number',
      'Should return count of deleted memories'
    );
  });
});
